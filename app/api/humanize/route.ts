...
export async function POST(request: Request) {
  ...
  try {
    const response = await fetch(`${base}/chat/completions`, { method: "POST", headers: { Authorization: `Bearer ${apiKey}`, "Content-Type": "application/json" }, body: JSON.stringify({ model: config?.textModel || process.env.AI_TEXT_MODEL || "gpt-4o", temperature: 0.8, messages: [{ role: "system", content: "请对下面的中文文章做自然化编辑：保留事实、结构和观点，减少模板化表达，加入更自然的句式变化，只输出处理后的 Markdown 正文。" }, { role: "user", content }] }), signal: AbortSignal.timeout(20000) });
