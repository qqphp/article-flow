...
import { fetchPptTask, resolveZhihuSecret, zhihuErrorPayload } from "../../../../lib/zhihu";

export const dynamic = "force-dynamic";

export async function POST(request: Request) {
  const body = await request.json().catch(() => ({})) as { taskId?: string; config?: { zhihuAccessSecret?: string } };
  ...
  try {
    return NextResponse.json(await fetchPptTask(secret, taskId));
  } catch (error) {
