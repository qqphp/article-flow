...
import { BookOpen, Bot, Check, ChevronRight, CircleHelp, Clock3, Eye, FileText, Image as ImageIcon, KeyRound, LayoutDashboard, Link2, Menu, MoreHorizontal, PenLine, Play, Plus, RefreshCw, Rocket, Search, Send, Settings2, Sparkles, Trash2, Wand2, X, Youtube, Zap, ScrollText } from "lucide-react";
...
function WritePage({ topic, setTopic, style, setStyle, searchOn, setSearchOn, running, start, generated, humanized, setHumanized, images, setImages, article, setArticle, notify }: any) {
  ...
  const handleImages = async () => {
    ...
    try {
      const response = await fetch("/api/article-images", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ title: displayTitle, articleId: article?.articleId, config: getConfig() }) });
      const data = await response.json();
      if (!response.ok || !data.article?.layoutContent) throw new Error(data.error || "配图失败");
      setArticle({ ...data.article, selectedTitle: article.selectedTitle }); setImages(true); setPreviewTab("layout");
