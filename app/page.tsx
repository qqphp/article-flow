...
import { BookOpen, Bot, Check, ChevronRight, CircleHelp, Clock3, Eye, FileText, Image as ImageIcon, KeyRound, LayoutDashboard, Link2, Menu, MoreHorizontal, PenLine, Play, Plus, RefreshCw, Rocket, Search, Send, Settings2, Sparkles, Wand2, X, Youtube, Zap, ScrollText } from "lucide-react";
...
function WritePage({ topic, setTopic, style, setStyle, searchOn, setSearchOn, running, start, generated, humanized, setHumanized, images, setImages, article, setArticle, notify }: any) {
  ...
  const paragraphImages = Array.isArray(article?.paragraphImages) ? article.paragraphImages.filter((image: any) => image?.url) : [];
  useEffect(() => {
    if (!operation) return;
  ...
  const handleImages = async () => {
    ...
    try {
      const response = await fetch("/api/article-images", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ title: displayTitle, content: article?.humanizedContent && humanized ? article.humanizedContent : article?.content, articleId: article?.articleId, config: getConfig() }) });
      const data = await response.json();
      if (!response.ok) throw new Error(data.error || "配图失败");
      setArticle({ ...article, imageUrl: data.coverUrl || null, paragraphImages: data.paragraphImages || [] }); setImages(true); setPreviewTab("images");
  ...
  const renderLayoutContent = (content: string) => {
    const blocks = normalizeMarkdown(content || "暂无内容").split(/\n\s*\n/).filter(Boolean);
    const imagesByBlock = new Map<number, any[]>();
    const usedBlocks = new Set<number>();
    paragraphImages.forEach((image: any, index: number) => {
      const matchedBlock = image.anchor ? blocks.findIndex((block, blockIndex) => !usedBlocks.has(blockIndex) && block.includes(image.anchor)) : -1;
      const fallbackBlock = Math.min(blocks.length - 1, Math.max(0, Math.floor(((index + 1) * blocks.length) / (paragraphImages.length + 1))));
      const blockIndex = matchedBlock >= 0 ? matchedBlock : fallbackBlock;
      usedBlocks.add(blockIndex);
      imagesByBlock.set(blockIndex, [...(imagesByBlock.get(blockIndex) || []), image]);
    });
    return <>{blocks.map((block, index) => <div key={`${index}-${block.slice(0, 20)}`}><ReactMarkdown remarkPlugins={[remarkGfm]}>{block}</ReactMarkdown>{imagesByBlock.get(index)?.map((image) => <figure className="layout-paragraph-image" key={image.url}><img src={image.url} alt="段落配图"/></figure>)}</div>)}</>;
  };
